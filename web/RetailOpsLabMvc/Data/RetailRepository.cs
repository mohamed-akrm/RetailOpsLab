...
namespace RetailOpsLabMvc.Data;
...
public class RetailRepository
{
    ...
    public async Task<DashboardViewModel> GetDashboardNumbersAsync()
    {
        ...
        if (await reader.ReadAsync())
        {
            ...
            model.TotalSales = GetDecimal(reader, "TotalSales");
        }
        return model;
    }
    ...
    public async Task<List<SalesReportRow>> GetSalesReportAsync(DateTime fromDate, DateTime toDate)
    {
        var rows = new List<SalesReportRow>();

        using var connection = CreateConnection();
        using var command = new SqlCommand("sales.usp_GetSalesReport", connection);
        command.CommandType = CommandType.StoredProcedure;
        ...
        while (await reader.ReadAsync())
        {
            rows.Add(new SalesReportRow
            {
                CustomerId = GetInt(reader, "CustomerId"),
                CustomerName = GetString(reader, "CustomerName"),
                NumberOfOrders = GetInt(reader, "NumberOfOrders"),
                TotalItemsSold = GetInt(reader, "TotalItemsSold"),
                TotalSales = GetDecimal(reader, "TotalSales")
            });
        }
    ...
    public async Task<FunctionLabViewModel> RunFunctionLabAsync(FunctionLabViewModel input)
    {
        ...
        using (var command = new SqlCommand("inventory.usp_GetRelatedProducts", connection))
        {
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.AddWithValue("@ProductId", model.ProductId);
            command.Parameters.AddWithValue("@CategoryId", model.CategoryId);
            ...
            while (await reader.ReadAsync())
            {
                model.RelatedProducts.Add(new RelatedProductViewModel
                {
                    ProductId = GetInt(reader, "ProductId"),
                    ProductName = GetString(reader, "ProductName"),
                    UnitPrice = GetDecimal(reader, "UnitPrice")
